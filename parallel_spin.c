...
using locks. However, retrieve doesn't need locks in order to get keys from the hashtable parallely,
...
void insert(int key, int val) {
	...
	pthread_spin_lock(&spinlock[i]); //acquire lock
	e->next = table[i];
	e->key = key;
	e->val = val;
	table[i] = e;
	pthread_spin_unlock(&spinlock[i]); // release lock
}
